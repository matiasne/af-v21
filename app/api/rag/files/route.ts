...
function detectDependencies(
  content: string,
  taskMetadata?: { dependsOn?: string[]; blocks?: string[] },
): DetectedDependency[] {
  ...
  try {
    ...
    if (!corpusName) {
      return NextResponse.json(
        {
          error:
            "corpusName query parameter is required (or use listCorpora=true)",
        },
        { status: 400 },
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "getOrCreateCorpus") {
      ...
      const corpus =
        await getRagRepository().getOrCreateCorpus(corpusDisplayName);
    ...
    if (action === "uploadDocument") {
        ...
        try {
          ...
          const taskNode: TaskNode = {
            ...
            cleanArchitectureArea:
              taskMetadata.cleanArchitectureArea || "infrastructure",
            projectId: projectId,
          ...
          console.log(
            "[RAG API] TaskNode to store:",
            JSON.stringify(taskNode, null, 2),
          );
          ...
          if (success) {
            console.log(
              "[RAG API] Task stored in Neo4j successfully:",
              displayName,
            );
          } else {
            console.log(
              "[RAG API] Failed to store task in Neo4j:",
              displayName,
            );
          }
          ...
          if (taskMetadata.epicId) {
            ...
            const linkSuccess = await graphRAGRepository.linkTaskToEpic(
              displayName,
              taskMetadata.epicId,
              projectId,
            );
          ...
          console.log(
            "[RAG API] Searching for similar tasks to create relationships...",
          );
          try {
            const similarTasks = await getRagRepository().searchFiles(
              content,
              corpusName,
            );

            console.log(
              `[RAG API] Found ${similarTasks.length} similar tasks from Pinecone`,
            );
            ...
            for (const similar of similarTasks) {
              ...
              if (
                similar.id === displayName ||
                similar.relevanceScore < SIMILARITY_THRESHOLD
              ) {
                continue;
              }

              console.log(
                `[RAG API] Creating SIMILAR_TO relationship: ${displayName} -> ${similar.id} (score: ${similar.relevanceScore.toFixed(2)})`,
              );
            ...
            console.log(
              `[RAG API] Created ${relationshipsCreated} SIMILAR_TO relationships`,
            );
          } catch (similarError) {
            console.error(
              "[RAG API] Error creating similar task relationships:",
              similarError,
            );
          ...
          try {
            ...
            console.log(
              `[RAG API] Detected ${detectedDeps.length} potential dependencies`,
            );
            ...
            for (const dep of detectedDeps) {
              console.log(
                `[RAG API] Processing ${dep.type} -> "${dep.targetReference}"`,
              );
              ...
              if (isExplicitTaskId) {
                ...
                console.log(
                  `[RAG API] Creating direct relationship with task ID: ${dep.targetReference}`,
                );
                ...
                if (relSuccess) {
                  depsCreated++;
                  console.log(
                    `[RAG API] Created ${dep.type} relationship: ${displayName} -> ${dep.targetReference}`,
                  );
                } else {
                  console.log(
                    `[RAG API] Failed to create relationship (target task may not exist in Neo4j): ${dep.targetReference}`,
                  );
                }
              } else {
                // Search for matching tasks in Pinecone by the reference (natural language detection)
                const matchingTasks = await getRagRepository().searchFiles(
                  dep.targetReference,
                  corpusName,
                );
                ...
                if (bestMatch) {
                  console.log(
                    `[RAG API] Found matching task: ${bestMatch.id} (score: ${bestMatch.relevanceScore.toFixed(2)})`,
                  );
                  ...
                  if (relSuccess) {
                    depsCreated++;
                    console.log(
                      `[RAG API] Created ${dep.type} relationship: ${displayName} -> ${bestMatch.id}`,
                    );
                  }
                } else {
                  console.log(
                    `[RAG API] No matching task found for: "${dep.targetReference}"`,
                  );
                }
              }
            }
            ...
            console.log(
              `[RAG API] Created ${depsCreated} dependency relationships`,
            );
          } catch (depError) {
            console.error(
              "[RAG API] Error creating dependency relationships:",
              depError,
            );
            // Continue - main task storage was successful
          }
          ...
          console.error(
            "[RAG API] ========== NEO4J TASK STORAGE ERROR ==========",
          );
          console.error("[RAG API] Error storing task in Neo4j:", neo4jError);
          console.error(
            "[RAG API] ========== NEO4J TASK STORAGE ERROR END ==========",
          );
      ...
      if (epicMetadata && projectId) {
        try {
          ...
          console.log(
            "[RAG API] EpicNode to store:",
            JSON.stringify(epicNode, null, 2),
          );
          ...
          if (success) {
            console.log(
              "[RAG API] Epic stored in Neo4j successfully:",
              displayName,
            );
          } else {
            console.log(
              "[RAG API] Failed to store epic in Neo4j:",
              displayName,
            );
          }
          ...
          console.error(
            "[RAG API] ========== NEO4J EPIC STORAGE ERROR ==========",
          );
          console.error("[RAG API] Error storing epic in Neo4j:", neo4jError);
          console.error(
            "[RAG API] ========== NEO4J EPIC STORAGE ERROR END ==========",
          );
          // Continue - Pinecone upload was successful
        }
      }
    ...
    if (action === "createRelationship") {
      ...
      if (!sourceTaskId || !targetTaskId || !relationshipType || !projectId) {
        return NextResponse.json(
          {
            error:
              "sourceTaskId, targetTaskId, relationshipType, and projectId are required",
          },
          { status: 400 },
        );
      }
      ...
      console.log(
        `[RAG API] Creating ${relationshipType} relationship: ${sourceTaskId} -> ${targetTaskId}`,
      );
      ...
      return NextResponse.json({
        success: true,
        relationship: { sourceTaskId, targetTaskId, relationshipType },
      });
    }
    ...
    if (action === "deleteRelationship") {
      ...
      console.log(
        `[RAG API] Deleting ${relationshipType} relationship: ${sourceTaskId} -> ${targetTaskId}`,
      );
    ...
    if (action === "updateTask") {
      ...
      const displayName = taskId.startsWith("task-")
        ? taskId
        : `task-${taskId}`;
      ...
      if (updates.title || updates.description) {
        try {
          ...
          const taskContent = [
            ...
            updates.cleanArchitectureArea
              ? `Architecture Layer: ${updates.cleanArchitectureArea}`
              : "",
          ]
            .filter(Boolean)
            .join("\n\n");
          ...
          await getRagRepository().deleteDocumentByDisplayName(
            corpusName,
            displayName,
          );
          const document = await getRagRepository().uploadDocument(
          ...
          console.error(
            "[RAG API] Error updating task in Pinecone:",
            pineconeError,
          );
      ...
      try {
        const taskNode: TaskNode = {
          ...
          cleanArchitectureArea:
            updates.cleanArchitectureArea || "infrastructure",
          projectId: projectId,
        ...
        if (updates.dependencies && Array.isArray(updates.dependencies)) {
          ...
          for (const dep of updates.dependencies) {
            ...
            if (bestMatch) {
              ...
              console.log(
                `[RAG API] Created DEPENDS_ON: ${displayName} -> ${bestMatch.id}`,
              );
              ...
              console.log(
                `[RAG API] Created DEPENDS_ON: ${displayName} -> ${dep}`,
              );
            }
          }
        }
  ...
  try {
    ...
    if (!corpusName || !displayName) {
      return NextResponse.json(
        {
          error:
            "corpusName and displayName query parameters are required (or use deleteCorpus=true)",
        },
        { status: 400 },
      );
    }
