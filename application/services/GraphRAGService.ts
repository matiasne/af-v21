...
export class GraphRAGService {
  ...
  async search(
    ...
    try {
      ...
      console.log(`[GraphRAG Service] Found ${vectorResults.length} vector results`);
      ...
      for (const result of vectorResults) {
        ...
        console.log(`[GraphRAG Service] Document ID from Pinecone: ${documentId}`);
        ...
        if (documentId) {
          ...
          if (opts.includeGraphContext) {
            console.log(`[GraphRAG Service] Getting graph context for task: ${documentId}`);
            const graphContext = await graphRAGRepository.getTaskWithRelationships(
              documentId,
              projectId
            );
            ...
            if (graphContext) {
              ...
              console.log(`[GraphRAG Service] No graph context found for: ${documentId}`);
            }
          }

          // Step 4: Get related tasks
          if (opts.includeRelatedTasks) {
            console.log(`[GraphRAG Service] Finding related tasks for: ${documentId}`);
            ...
            if (relatedTasks.length > 0) {
              ...
              console.log(`[GraphRAG Service] No related tasks found for: ${documentId}`);
            }
          }
        } else {
          console.log(`[GraphRAG Service] No document ID found, skipping Neo4j lookup`);
        }
      ...
      console.log("[GraphRAG Service] ========== SEARCH END (ERROR) ==========");
      throw error;
    }
  }
  ...
  formatForLLMContext(results: EnrichedRAGResult[]): string {
    ...
    sections.push("SIMILAR EXISTING TASKS/CONTEXT FOUND IN PROJECT (from GraphRAG search):");
    sections.push("---");
    ...
    results.forEach((result, index) => {
      ...
      if (result.relatedTasks && result.relatedTasks.length > 0) {
        ...
        result.relatedTasks.slice(0, 3).forEach((task) => {
          sections.push(`  - ${task.title} (${task.category}, ${task.priority})`);
        });
      }
    ...
    sections.push("1. Mention to the user that similar tasks may already exist");
    sections.push("2. Explain what you found and how it relates to their request");
    sections.push(
